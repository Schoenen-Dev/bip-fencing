...
export default function Quotation() {
  ...
  const fetchQuotations = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/get_quotations.php`, {
        headers: getHeaders(),
      });
  ...
  const fetchQuotationDetails = async (id) => {
    try {
      const res = await fetch(`${API_BASE}/get_quotation.php?id=${id}`, {
        headers: getHeaders(),
      });
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editId
        ? `${API_BASE}/update_quotation.php`
        : `${API_BASE}/add_quotation.php`;
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/delete_quotation.php?id=${deleteId}`,
