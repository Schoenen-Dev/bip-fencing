...
export default function Dashboard() {
  ...
  const statCards = [
    { label: 'Total Revenue', value: totalRevenue.toFixed(2), icon: 'bi-currency-rupee', color: 'green', unit: '₹ ' },
    { label: 'Purchase Expense', value: totalExpense.toFixed(2), icon: 'bi-receipt', color: 'red', unit: '₹ ' },
    { label: 'Profit', value: profit.toFixed(2), icon: 'bi-graph-up', color: 'blue', unit: '₹ ' },
    { label: 'Employees', value: totalEmployees, icon: 'bi-people', color: 'orange' },
    { label: 'Present Today', value: present, icon: 'bi-person-check', color: 'teal' },
    { label: 'Clients', value: data.clients.length, icon: 'bi-briefcase', color: 'pink' },
  ];
    ...
    <div className="dashboard-container">
      ...
      <div className="stats-grid">
        {statCards.map((card, i) => (
          <div key={i} className="stat-card" style={{ background: cardColors[card.color].bg }}>
            ...
            <div className="stat-card-content">
              ...
              <div className="stat-card-value">
                {card.unit}{typeof card.value === 'number' ? card.value.toLocaleString('en-IN') : card.value}
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="chart-section">
        <div className="section-header">
          <h3><i className="bi bi-bar-chart-fill"></i> Revenue vs Expense</h3>
        ...
        <div className="chart-legend">
          <span><i className="bi bi-circle-fill" style={{ color: '#10b981' }}></i> Revenue</span>
          <span><i className="bi bi-circle-fill" style={{ color: '#ef4444' }}></i> Expense</span>
        </div>
      </div>
      ...
      <div className="recent-grid">
        ...
        <div className="recent-card">
          <div className="recent-header">
            <h3><i className="bi bi-receipt"></i> Recent Invoices</h3>
            <button className="view-link" onClick={() => window.location.href = '/billing'}>View all →</button>
          </div>
          <div className="recent-list">
            {data.invoices.length === 0 ? (
              <div className="empty-state">No invoices yet</div>
            ) : (
              data.invoices.slice(-4).reverse().map((inv, idx) => (
                <div key={idx} className="recent-item">
                  <div className="recent-info">
                    <div className="recent-title">{inv.invoiceNo}</div>
                    <div className="recent-meta">{inv.clientName || 'Walk-in'}</div>
                  </div>
                  <div className="recent-amount revenue">{inr(inv.total || 0)}</div>
                </div>
              ))
            )}
          </div>
        ...
        <div className="recent-card">
          <div className="recent-header">
            <h3><i className="bi bi-cart"></i> Recent Purchases</h3>
            <button className="view-link" onClick={() => window.location.href = '/purchase-bill'}>View all →</button>
          </div>
          <div className="recent-list">
            {data.purchases.length === 0 ? (
              <div className="empty-state">No purchases yet</div>
            ) : (
              data.purchases.slice(-4).reverse().map((pur, idx) => (
                <div key={idx} className="recent-item">
                  <div className="recent-info">
                    <div className="recent-title">{pur.billNo}</div>
                    <div className="recent-meta">{pur.company_name || 'Supplier'}</div>
                  </div>
                  <div className="recent-amount expense">{inr(pur.grandTotal || 0)}</div>
                </div>
              ))
            )}
          </div>
        ...
        <div className="recent-card">
          <div className="recent-header">
            <h3><i className="bi bi-file-text"></i> Recent Quotations</h3>
            <button className="view-link" onClick={() => window.location.href = '/quotation'}>View all →</button>
          </div>
          ...
          {quoteSummary.totalQuotes > 0 && (
            <div className="quote-summary">
              ...
              <div className="quote-summary-item">
                <span>Discount</span>
                <strong className="discount">{inr(quoteSummary.totalDiscount)}</strong>
              </div>
              <div className="quote-summary-item">
                <span>Revenue</span>
                <strong className="revenue">{inr(quoteSummary.totalRevenue)}</strong>
              </div>
            </div>
          )}
          ...
          <div className="recent-list">
            {data.quotations.length === 0 ? (
              ...
              data.quotations.slice(-3).reverse().map((q, idx) => (
                <div key={idx} className="recent-item">
                  <div className="recent-info">
                    <div className="recent-title">{q.quoteNo}</div>
                    <div className="recent-meta">{q.clientName || 'New quote'}</div>
                  </div>
      ...
      <div className="bottom-grid">
        ...
        <div className="info-card">
          <div className="info-header">
            <h3><i className="bi bi-people"></i> Employee Status</h3>
          </div>
        ...
        {lowStock.length > 0 && (
          <div className="info-card alert">
            ...
            <div className="stock-list">
              ...
              {lowStock.length > 5 && (
                <div className="stock-more">+{lowStock.length - 5} more products</div>
              )}
            </div>
          </div>
        )}
