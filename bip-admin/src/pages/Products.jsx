...
const emptyForm = {
  ...
  sellingPrice: '', stockQty: '', minStock: '', description: '',
};
...
export default function Products() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetchProducts();
      setEditingId(null); setForm(emptyForm); setAddStockQty('');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (err) { setError('❌ ' + err.message); }
  };

  const handleEdit = (p) => {
    setForm({ productName: p.productName, sku: p.sku, category: p.category,
      unit: p.unit, sellingPrice: p.sellingPrice, stockQty: p.stockQty,
      minStock: p.minStock, description: p.description });
    setEditingId(p.id); setAddStockQty('');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const cancelEdit = () => { setEditingId(null); setForm(emptyForm); setAddStockQty(''); setError(''); };
  ...
  return (
    <>
      ...
      <h2 style={{ fontSize: 24, fontWeight: 700, color: '#111827', marginBottom: 20 }}>🗂️ Products</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="pf-section">
          ...
          <div className="pf-grid-3" style={{ marginBottom: 16 }}>
            <div className="pf-field">
              <label className="pf-label">Product Name <span className="req">*</span></label>
              <input className="pf-input" name="productName" value={form.productName} onChange={handleChange} placeholder="e.g. Chain Link Fence Roll" required />
            </div>
            <div className="pf-field">
              <label className="pf-label">SKU / Code</label>
              <input className="pf-input" name="sku" value={form.sku} onChange={handleChange} placeholder="e.g. PRD-001" />
            </div>
            <div className="pf-field">
              <label className="pf-label">Category</label>
              <input className="pf-input" name="category" value={form.category} onChange={handleChange} placeholder="e.g. Fencing" />
            </div>
          </div>
          <div className="pf-grid-4">
            ...
            <div className="pf-field">
              ...
              <input type="number" className="pf-input" name="sellingPrice" value={form.sellingPrice} onChange={handleChange} onWheel={e => e.target.blur()} placeholder="0.00" min="0" step="0.01" />
            </div>
          </div>
        </div>
        ...
        <div className="pf-section">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: editingId !== null ? '1fr 1fr 1fr' : '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div className="pf-field">
              <label className="pf-label">Current Stock Qty</label>
              {(() => {
                const extra    = Number(addStockQty) || 0;
                const base     = Number(form.stockQty) || 0;
                const total    = editingId !== null && extra > 0 ? base + extra : base;
                const display  = editingId !== null && extra > 0 ? total : form.stockQty;
                const hasAdded = editingId !== null && extra > 0;
                const isOut    = total === 0 && form.stockQty !== '';
                const isLow    = total > 0 && total <= Number(form.minStock) && form.minStock !== '';
                return (
                  <input type="number" className="pf-input" name="stockQty"
                    value={display}
                    onChange={hasAdded ? undefined : handleChange}
                    readOnly={hasAdded}
                    onWheel={e => e.target.blur()} placeholder="e.g. 100" min="0"
                    style={hasAdded ? { borderColor: '#22c55e', background: '#f0fdf4', fontWeight: 700, color: '#15803d', cursor: 'default' }
                      : isOut ? { borderColor: '#ef4444', background: '#fff5f5' }
                      : isLow ? { borderColor: '#f59e0b', background: '#fffdf0' } : {}} />
                );
              })()}
            </div>
            {editingId !== null && (
              <div className="pf-field">
                <label className="pf-label" style={{ color: '#2563eb' }}>➕ Add Stock Qty</label>
                <input type="number" className="pf-input" value={addStockQty}
                  onChange={e => setAddStockQty(e.target.value)}
                  onWheel={e => e.target.blur()} placeholder="e.g. 50" min="0"
                  style={{ borderColor: '#3b82f6', background: '#eff6ff' }} />
              </div>
            )}
            <div className="pf-field">
              <label className="pf-label">Minimum Stock Alert</label>
              <input type="number" className="pf-input" name="minStock" value={form.minStock} onChange={handleChange} onWheel={e => e.target.blur()} placeholder="e.g. 10" min="0" />
              <span style={{ fontSize: 11, color: '#9ca3af' }}>Alert triggers when stock ≤ this value</span>
            </div>
          </div>
          <div className="pf-field">
            <label className="pf-label">Description</label>
            <textarea className="pf-textarea" name="description" value={form.description} onChange={handleChange} rows={3} placeholder="Product description, specifications, notes..." />
          </div>
        </div>

        <div style={{ display: 'flex', gap: 10, marginBottom: 28 }}>
          <button type="submit" className="pf-btn-save">{editingId !== null ? '✓ Update Product' : '+ Save Product'}</button>
      ...
      <div className="pf-section" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontSize: 12, color: '#6b7280' }}>{loading ? 'Loading…' : `${products.length} product${products.length !== 1 ? 's' : ''}`}</span>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table className="pf-table">
            <thead>
              <tr><th>#</th><th>Name</th><th>SKU</th><th>Category</th><th>Unit</th><th>Price (INR)</th><th>Stock</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="pf-empty">Loading products…</td></tr>
              ) : products.length === 0 ? (
                <tr><td colSpan={8} className="pf-empty">No products yet. Add your first product above.</td></tr>
              ) : products.map((p, i) => {
                const qty = Number(p.stockQty), min = Number(p.minStock);
                const rowClass = qty === 0 ? 'row-alert-out' : qty <= min && min > 0 ? 'row-alert-low' : '';
                return (
                  <tr key={p.id} className={rowClass}>
                    <td style={{ color: '#6b7280', fontWeight: 600 }}>{i + 1}</td>
                    <td>
                      <div style={{ fontWeight: 600 }}>{p.productName}</div>
                      {p.description && <div style={{ fontSize: 11, color: '#9ca3af' }}>{p.description.substring(0, 40)}{p.description.length > 40 ? '…' : ''}</div>}
                    </td>
...
function badge(color) {
  ...
  return { ...map[color], display: 'inline-block', borderRadius: 20, padding: '3px 10px', fontSize: 12, fontWeight: 700 };
}