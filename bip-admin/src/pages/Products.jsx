...
export default function Products() {
  ...
  const [addStockQty, setAddStockQty] = useState('');  // extra stock to add on edit
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      setError('❌ Could not load products. Is XAMPP running?');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleEdit = (p) => {
    setForm({
      ...
      unit: p.unit,  sellingPrice: p.sellingPrice,
  ...
  return (
    <>
      <style>{`
        ...
        .pf-input::placeholder, .pf-textarea::placeholder { color: #9ca3af; font-size: 13px; }
        ...
        .pf-btn-save { background: #2563eb; color: #fff; border: none; border-radius: 7px; padding: 10px 24px; font-size: 13.5px; font-weight: 600; cursor: pointer; transition: background 0.15s; }
        ...
        @media (max-width: 768px) { .pf-grid-4 { grid-template-columns: 1fr 1fr; } .pf-grid-3 { grid-template-columns: 1fr 1fr; } .pf-grid-2 { grid-template-columns: 1fr 1fr; } }
        @media (max-width: 480px) { .pf-grid-4, .pf-grid-3, .pf-grid-2 { grid-template-columns: 1fr; } .pf-table th, .pf-table td { padding: 8px; font-size: 11px; } }
      `}</style>
...
{/* ── Error Banner ── */}
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="pf-grid-4">
            <div className="pf-field">
              ...
              <select className="pf-select" name="unit" value={form.unit} onChange={handleChange}>
                {['Pcs', 'Kg', 'Meter', 'Roll', 'Box', 'Set', 'Liter'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
        ...
        <div className="pf-section">
          ...
          {/* Grid: 2 cols normally, 3 cols when editing (adds "Add Stock Qty") */}
          <div style={{
            display: 'grid',
            gridTemplateColumns: editingId !== null ? '1fr 1fr 1fr' : '1fr 1fr',
            gap: 16,
            marginBottom: 16,
          }}>
            {/* Current Stock Qty — updates live when Add Stock Qty is typed */}
            <div className="pf-field">
                ...
                return (
                  <>
                    <input
                      ...
                      style={
                        hasAdded
                          ? { borderColor: '#22c55e', background: '#f0fdf4', fontWeight: 700, color: '#15803d', cursor: 'default' }
                          : isOut
            ...
            {/* Add Stock Qty — only visible in edit mode */}
            {editingId !== null && (
              <div className="pf-field">
