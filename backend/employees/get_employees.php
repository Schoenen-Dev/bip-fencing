    ...
    // If simple=1, return just the employee list for dropdown (no pagination, no extra fields)
    if (isset($_GET['simple']) && $_GET['simple'] == 1) {
        list($whereClause, $branchParams) = branchFilter($authUser);
        $sql = "SELECT id, employee_name, emp_id FROM employees $whereClause ORDER BY employee_name ASC";
        $stmt = $db->prepare($sql);
        if ($branchParams) {
